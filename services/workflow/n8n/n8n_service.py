...
class N8nService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    error_msg = f"Workflow execution failed with status {response.status_code}: {response.text}"
            ...
            raise Exception(error_msg,status_code=500)
    ...
    async def _create_workflow_from_template(self, workspace: str, segment: str, data: Dict[str, Any]) -> WorkflowExecuteResponse:
        ...
        try:
            ...
            if not template_workflow:
                return WorkflowExecuteResponse(
                    success=False,
                    error=f"No template workflow found for workspace '{workspace}' and segment '{segment}'",
            ...
            if not cloned_workflow:
                return WorkflowExecuteResponse(
                    success=False,
                    error="Failed to clone template workflow",
            ...
            error_msg = f"Error creating workflow from template: {str(e)}"
            self.logger.error(error_msg)
            return WorkflowExecuteResponse(
                success=False,
                error=str(e),
    ...
    async def _find_template_workflow(self, segment: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                self.logger.warning(f"No template workflow found with tags: template, {workspace}, {segment}")
                return None
                
        except Exception as e:
            self.logger.error(f"Error finding template workflow: {str(e)}")
            return None
    ...
    async def _clone_workflow(self, workflow_id: str, new_name: str, workspace: str, segment: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error cloning workflow: {str(e)}")
            return None
    ...
    async def _update_workflow_trigger(self, workflow_id: str, new_trigger_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Error updating workflow trigger: {str(e)}")
            return None
    ...
    async def _activate_workflow_api(self, workflow_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error activating workflow: {str(e)}")
            return False
    ...
    async def get_template_workflows(self) -> WorkflowTemplatesResponse:
        ...
        try:
            ...
            error_msg = f"Error fetching template workflows: {str(e)}"
            self.logger.error(error_msg)
            return WorkflowTemplatesResponse(
                success=False,
                templates=[],
